...
brand_idx = (
    brand_options.index(st.session_state["saved_brand"])
    if st.session_state["saved_brand"] in brand_options
    else 0
)

cat_options = ["전체"] + CATEGORIES
cat_idx = (
    cat_options.index(st.session_state["saved_category"])
    if st.session_state["saved_category"] in cat_options
    else 0
)

target_options = ["전체", "제목만", "내용만", "제목+내용"]
target_idx = (
    target_options.index(st.session_state["saved_target"])
    if st.session_state["saved_target"] in target_options
    else 0
)

sort_options = ["최신순", "조회순", "공감순"]
sort_idx = (
    sort_options.index(st.session_state["saved_sort"])
    if st.session_state["saved_sort"] in sort_options
    else 0
)
...
with st.container(border=True):
    ...
    with col4:
        st.text_input(
            "검색어",
            value=st.session_state["saved_keyword"],
...
active_filters = (
    brand_filter != "전체" or category_filter != "전체" or bool(keyword.strip())
)
count_txt = "🔎 검색 결과" if active_filters else "전체 게시글"
    ...
    for post in page_posts:
        with st.container(border=True):
            ...
            with top_l:
                ...
                st.markdown(
                    f"{accent_bar}{brand_badge}{cat_badge}</div>",
                    unsafe_allow_html=True,
                )
            with top_r:
            ...
            st.caption(
                f"{model_txt}작성자: {post['author'] or '익명'}  |  {post['created_at']}"
            )
