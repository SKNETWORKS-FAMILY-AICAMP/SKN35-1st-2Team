...
def _search_address(address):
    ...
    res = requests.get(url, headers=_HEADERS, params={"query": address}, timeout=5)
...
def _search_keyword(keyword):
    ...
    res = requests.get(url, headers=_HEADERS, params={"query": keyword}, timeout=5)
    res.raise_for_status()
    docs = res.json().get("documents", [])
    if docs:
        return docs[0]["y"], docs[0]["x"]
...
def get_coords(address=None, keyword=None, delay=0.2):
    ...
    address, keyword 둘 다 없으면 (None, None) 반환.
    """
    lat, lng = None, None
